...
begin
    ...
    if exists (
        ...
        where lower(trim(nombre)) = lower(trim(p_nombre))
          and lower(trim(tipo)) = lower(trim(p_tipo))
          and id_usuario_propetario = p_id_usuario
    ) then
        signal sqlstate '45000'
...
begin
    ...
    select id_usuario_propetario, tipo
    into v_id_usuario, v_tipo
    from categoria
    where id_categoria = p_id_categoria;
    ...
    if exists (
        ...
        from categoria
        where lower(trim(nombre)) = lower(trim(p_nombre))
          and lower(trim(tipo)) = lower(trim(v_tipo))
          and id_usuario_propetario = v_id_usuario
