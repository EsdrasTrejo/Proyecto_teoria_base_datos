...
begin
    ...
    if (anio_periodo_fin < anio_periodo_inicio) or (anio_periodo_fin = anio_periodo_inicio and mes_periodo_fin < mes_periodo_inicio) then
    signal sqlstate '45000'
    ...
    and mes_inicio = mes_periodo_inicio and year_fin = anio_periodo_fin and mes_fin = mes_periodo_fin) then
    signal sqlstate '45000'
    set message_text = 'ya existe un presupuesto con ese nombre y periodo para este usuario';
    end if;

    insert into presupuesto(id_usuario, nombre, year_inicio, mes_inicio, year_fin, mes_fin, estado, creado_user, creado_fecha)
...
anio_periodo_fin int, mes_periodo_fin int, p_modificado_por varchar(300))
begin
    ...
    and id_presupuesto <> p_id_presupuesto ) then  
    signal sqlstate '45000'
