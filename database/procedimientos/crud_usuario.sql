...
begin
    ...
    where lower(trim(correo)) = lower(trim(p_correo)) ) then
    signal sqlstate '45000'
...
create procedure sp_eliminar_usuario( p_id_usuario int, p_modificado_por varchar(300))
begin 
    ...
    if exists ( select 1 from usuario
    where id_usuario = p_id_usuario and estado = 0) then
    signal sqlstate '45000'
    ...
    -- opcional: bloquear si tiene presupuestos activos
    if exists ( select 1 from presupuesto
    where id_usuario = p_id_usuario and estado = 1 ) then
    signal sqlstate '45000'
    set message_text = 'no se puede desactivar el usuario porque tiene presupuestos activos';
    end if;

    update usuario  set estado = 0, modificado_user = trim(p_modificado_por), modificado_fecha = current_timestamp
...
begin
    ...
    if exists ( select 1 from usuario 
    where lower(trim(correo)) = lower(trim(p_correo)) and id_usuario <> p_id_usuario) then
    signal sqlstate '45000'
